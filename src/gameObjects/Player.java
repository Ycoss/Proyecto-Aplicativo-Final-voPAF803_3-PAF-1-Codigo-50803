...
public class Player extends MovingObject{
	...
	public void update(float dt) 
	{
		...
		if(doubleScoreOn)
			doubleScoreTime += (long)dt;
		
		if(fastFireOn) {
			fireSpeed = Constants.FIRERATE / 2;
			fastFireTime += (long)dt;
		}else {
			fireSpeed = Constants.FIRERATE;
		}
		
		if(doubleGunOn)
			doubleGunTime += (long)dt;
		...
		// Mensaje de depuración para verificar los valores en cada frame
		// System.out.println("Valores: MLB: " + input.MouseInput.MLB + 
		//		", SHOOT: " + KeyBoard.SHOOT + 
		//		", fireRate: " + fireRate + 
		//		", fireSpeed: " + fireSpeed + 
		//		", spawning: " + spawning);
		
		if((KeyBoard.SHOOT || input.MouseInput.MLB) &&  fireRate > fireSpeed && !spawning)
		{
			// System.out.println("¡Disparando! MLB: " + input.MouseInput.MLB + ", SHOOT: " + KeyBoard.SHOOT);
			
			if(doubleGunOn) {
				// Punto de origen base bien adelante del centro del jugador
                Vector2D baseFirePosition = getCenter().add(heading.normalize().scale(width * 0.80));

                // Separación lateral (el valor actual debería estar bien si la dirección es correcta)
                double lateralSeparation = width / 2.2;

                // Dirección del heading: this.angle - Math.PI/2
                // Dirección perpendicular 1: this.angle
                // Dirección perpendicular 2: this.angle - Math.PI

                // Asegurar que los vectores de desplazamiento comiencen con magnitud distinta de cero antes de setDirection
                Vector2D offset1 = new Vector2D(1, 0).setDirection(this.angle).scale(lateralSeparation);
                Vector2D offset2 = new Vector2D(1, 0).setDirection(this.angle - Math.PI).scale(lateralSeparation);

                Vector2D spawnPos1 = new Vector2D(baseFirePosition).add(offset1);
                Vector2D spawnPos2 = new Vector2D(baseFirePosition).add(offset2);
                                
                Laser l = new Laser(spawnPos1, new Vector2D(heading), Constants.LASER_VEL, angle, Assets.currentPlayerLaserTexture, gameState, false);
                Laser r = new Laser(spawnPos2, new Vector2D(heading), Constants.LASER_VEL, angle, Assets.currentPlayerLaserTexture, gameState, false);
                
                gameState.getMovingObjects().add(0, l);
                gameState.getMovingObjects().add(0, r);
				
			}else {
				// Disparo único: Revertir a una distancia de spawn similar a la base del doble cañón.
                Vector2D spawnPosition = getCenter().add(new Vector2D(heading).normalize().scale(width * 0.80)); 
				gameState.getMovingObjects().add(0,new Laser(
						spawnPosition, // Usar esta posición directamente
						new Vector2D(heading),
						Constants.LASER_VEL,
						angle,
						Assets.currentPlayerLaserTexture,
						gameState,
						false
						));
			}
	...
	public void draw(Graphics g) {
		...
		if(doubleGunOn)
			g2d.drawImage(Assets.currentDoubleGunPlayerTexture, at, null);
		else
